...
`transform_path_points` transforms one or more explicit endpoint/control-point selections on an existing path. Query first with `query_path_nodes`, then pass `pointSelector.points` entries such as `{ "segmentIndex": 2, "point": "c1" }` plus either `transform: { "type": "translate", "dx": -2, "dy": 1 }` or `transform: { "type": "set_absolute", "points": [{ "x": 116, "y": 36 }] }`. The `set_absolute` target points are matched to selections by array order and preserve relative path command forms when present. The tool preserves the path element id and object tree, snapshots before successful writes, returns previous/next `d`, and uses direct active-window `d` sync.
