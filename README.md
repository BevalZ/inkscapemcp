...
`transform_path_points` translates one or more explicit endpoint/control-point selections on an existing path. Query first with `query_path_nodes`, then pass `pointSelector.points` entries such as `{ "segmentIndex": 2, "point": "c1" }` plus `transform: { "type": "translate", "dx": -2, "dy": 1 }`. It preserves the path element id and object tree, snapshots before successful writes, returns previous/next `d`, and uses direct active-window `d` sync.
