...
`validate_path_data` is a read-only preflight helper for raw SVG path `d` strings. It returns compact segment counts, command counts, unsupported-command count, relative/absolute command counts, and editable point summaries without requiring `docId`, reading workspace files, creating snapshots, writing logs, or refreshing Inkscape. Validation failures include actionable diagnostics when available: command, segment index, command/token index, parameter counts, missing parameter count, source offset, and invalid text fragments. Set `requireMoveTo: false` to validate append-style fragments such as `L10 10`; diagnostics then use segment indexes local to the supplied fragment.

`edit_path_nodes` applies compact node-level edits to an existing path's `d` attribute. It can move or set exact endpoint/control-point coordinates, insert structured segments, and delete segments. For safe round-tripping it supports paths made from `M`, `L`, `H`, `V`, `C`, `Q`, and `Z` commands; `H`/`h` and `V`/`v` endpoints preserve their command storage and reject edits that would require converting them to `L`/`l`. Use `replace_path_data` for more complex SVG path commands such as arcs.

`transform_path_points` transforms endpoint/control-point selections on an existing path. Query first with `query_path_nodes`, then pass explicit `pointSelector.points` entries such as `{ "segmentIndex": 2, "point": "c1" }`, a bounded selector such as `{ "type": "bbox", "minX": 10, "minY": 10, "maxX": 40, "maxY": 30, "pointTypes": ["end", "c1"] }`, a segment range selector such as `{ "type": "segment_range", "startSegmentIndex": 1, "endSegmentIndex": 3, "pointTypes": ["end"] }`, a segment list selector such as `{ "type": "segment_list", "segmentIndexes": [1, 3, 5], "pointTypes": ["end"] }`, a command selector such as `{ "type": "command", "commands": ["H", "V", "C", "c"], "pointTypes": ["end", "c1", "c2"] }`, a point-type selector such as `{ "type": "point_type", "pointTypes": ["end"] }`, a nearest-point selector such as `{ "type": "nearest", "x": 118, "y": 34, "pointTypes": ["c1", "end"], "maxDistance": 6 }`, or a radius selector such as `{ "type": "radius", "x": 118, "y": 34, "radius": 8, "pointTypes": ["c1", "c2"] }`. Bbox, nearest, and radius coordinates are absolute SVG user units; bbox includes edge points, segment ranges are inclusive path segment indexes, segment lists target non-contiguous segment indexes, command selectors target case-sensitive supported path command names, point-type selectors target every editable point of the required point kinds, nearest selects exactly one editable point, and radius selects every editable point within the inclusive circular distance. Selector `pointTypes` defaults to `["end", "c1", "c2"]` where optional, and resolved selectors use deterministic path-order tie-breaks/order. Use `transform: { "type": "translate", "dx": -2, "dy": 1 }`, `transform: { "type": "set_absolute", "points": [{ "x": 116, "y": 36 }] }`, `transform: { "type": "set_relative", "points": [{ "x": 16, "y": 6 }] }`, `transform: { "type": "scale", "origin": { "x": 100, "y": 30 }, "sx": 1.1, "sy": 0.9 }`, `transform: { "type": "rotate", "origin": { "x": 100, "y": 30 }, "angleDegrees": 15 }`, `transform: { "type": "reflect", "axis": "vertical", "origin": { "x": 100, "y": 30 } }`, `transform: { "type": "reflect_line", "origin": { "x": 100, "y": 30 }, "angleDegrees": 45 }`, or `transform: { "type": "skew", "axis": "x", "origin": { "x": 100, "y": 30 }, "angleDegrees": 12 }`. Set-transform target points are matched to resolved selections by array order; `set_absolute` uses absolute SVG coordinates, `set_relative` uses coordinates relative to each selected segment's current base point, and `scale`/`rotate`/`reflect`/`reflect_line`/`skew` map selected absolute point coordinates around the explicit origin before writing them back into the existing path command storage. `H`/`h` endpoints can only be written when the resulting y remains representable by the horizontal command, and `V`/`v` endpoints can only be written when the resulting x remains representable by the vertical command. `reflect.axis` supports `vertical` for the line `x = origin.x` and `horizontal` for the line `y = origin.y`; `reflect_line.angleDegrees` reflects across the infinite line through `origin` at that angle in SVG user units; `skew.axis` supports `x` for horizontal shear and `y` for vertical shear. The tool preserves path command case, element id, and object tree, snapshots before successful writes, returns previous/next `d`, and uses direct active-window `d` sync.

`query_path_nodes` is read-only and returns the same editable segment boundary used by `edit_path_nodes`, including segment indexes, raw point values, absolute point positions, segment base points, and available point names. `H`/`h` endpoints report the segment base y, and `V`/`v` endpoints report the segment base x. It accepts `normalize: "none" | "absolute" | "relative"`; the default keeps the existing raw segment response, `normalize: "absolute"` adds a normalized segment view whose point values are absolute coordinates, and `normalize: "relative"` adds a normalized segment view whose point values are relative to each segment base point. It does not create snapshots or refresh Inkscape.
